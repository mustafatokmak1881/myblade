...
socket.id,85,85,boyut,guc,renk;

*/

var pattern = "p,-250,-250;p,100,100;p,200,400;";
//var pattern = "";
...
io.on("connection", function(socket){
	...
	alldatas = alldatas+socket.id+","+rand_x+","+rand_y+","+tip+","+can+","+hiz+","+way_x+","+way_y+";";
	io.emit("adduser",{
		'id' : socket.id,
		'x' : rand_x,
		'y'	: rand_y,
	...
	socket.on("disconnect",function(){
		console.log("Online: "+io.engine.clientsCount);
		var rege_x = new RegExp(socket.id+",(.*?),(.*?),(.*?),(.*?),(.*?),(.*?),(.*?);","g");
		var regexp_out = rege_x.exec(alldatas);
		if (regexp_out){
			var dataid_info = socket.id+","+regexp_out[1]+","+regexp_out[2]+","+regexp_out[3]+","+regexp_out[4]+","+regexp_out[5]+","+regexp_out[6]+","+regexp_out[7]+";";
